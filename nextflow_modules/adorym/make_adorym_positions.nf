...
moduleParams= new nextflow.script.ScriptBinding$ParamsMap(params.adorym_workflow.make_adorym_positions) // "navigate"/"select" the right section in the Yaml
...
process make_adorym_positions {
    publishDir params.outputfolder+"/probe_positions/", mode: 'copy' // location to save the outputs, copying as the default is to symbolically link!
        ...
        script: //default Bash, see https://www.nextflow.io/docs/latest/process.html#script
            """
            /opt/anaconda3/envs/tompekin-basic/bin/python $projectDir/bin/adorym/make_adorym_positions.py $expandedParameters
            """
    }